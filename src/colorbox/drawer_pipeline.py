...
def sample_antipodal_candidates_from_point_cloud(
    ...
    # Precompute distance matrix once (O(N^2) – OK for a few thousand points)
    dmat = np.linalg.norm(points[:, None, :] - points[None, :, :], axis=2)

    for _ in range(n_candidates):
        i = np.random.randint(N)
        # find neighbors within [min_pt_dist, max_pt_dist]
        dists = dmat[i]
        valid_mask = (dists > min_pt_dist) & (dists < max_pt_dist)
        valid_idx = np.where(valid_mask)[0]
        if valid_idx.size == 0:
            continue

        # choose a random neighbor in that band
        j = int(np.random.choice(valid_idx))
...
def build_diagram_and_context():
    """
    Uses your create_bimanual_IIWA14_with_table_and_initials_and_assets_and_cameras()
    to build the diagram and get a default context ready for evaluation.
    """
    builder, station = create_bimanual_IIWA14_with_table_and_initials_and_assets_and_cameras()
    diagram = builder.Build()
    context = diagram.CreateDefaultContext()

    # Render sensors (incl. camera point clouds) once
...
# Phase 1 keyframes: initial -> above handle (open) -> grasp (closed)
keyframes = [
    (X_WGinitial,           opened),
